    ...
    def predict_on_features(self, features, model=None):
        ...
        for x in tqdm(features, desc="Predicting on clips"):
            ...
            if x.shape[1] < window_size:
                pad = torch.zeros(
                    (1, window_size - x.shape[1], x.shape[2]),
                    dtype=x.dtype,
                    device=x.device
                )
            ...
            batch = []
            for i in range(0, x.shape[1] - window_size + 1, 1):
                batch.append(x[:, i:i + window_size, :])
...
def get_wav_paths(directory):
    """Return only WAV files from a dataset directory, sorted for reproducibility."""
    paths = sorted(Path(directory).glob("*.wav"))
    if not paths:
        raise RuntimeError(f"No WAV files found in dataset directory: {directory}")
    return [str(p) for p in paths]


def count_wav_files(directory):
    """Count only WAV files; ignore auxiliary files and directories."""
    return len(list(Path(directory).glob("*.wav")))


def make_fixed_feature_transform(target_timesteps, feature_dim):
    """
    Return a transform that guarantees a batch has shape
    (batch, target_timesteps, feature_dim).
    """
    def transform(x):
        x = np.asarray(x)
        if x.ndim != 3:
            raise ValueError(
                f"Expected feature batch with 3 dimensions "
                f"(batch, timesteps, features), got shape {x.shape}"
            )

        if x.shape[2] != feature_dim:
            raise ValueError(
                f"Feature dimension mismatch: expected {feature_dim}, "
                f"got {x.shape[2]} for array {x.shape}"
            )
        ...
        if x.shape[1] > target_timesteps:
            return x[:, :target_timesteps, :]

        padded = np.zeros(
            (x.shape[0], target_timesteps, feature_dim),
            dtype=x.dtype
        )
        padded[:, :x.shape[1], :] = x
        return padded
...
def validate_feature_file(path, expected_timesteps=None, expected_feature_dim=96):
    """Validate a feature .npy file and return its shape."""
    path = Path(path)
    if not path.exists():
    ...
    if arr.ndim != 3:
        raise ValueError(
            f"{path} has invalid shape {arr.shape}; expected 3 dimensions."
        )

    if expected_timesteps is not None and arr.shape[1] != expected_timesteps:
        raise ValueError(
            f"{path} has {arr.shape[1]} timesteps, expected {expected_timesteps}. "
            "Remove/regenerate the stale feature file instead of mixing shapes."
        )

    if arr.shape[2] != expected_feature_dim:
        raise ValueError(
            f"{path} has feature dimension {arr.shape[2]}, "
...
if __name__ == '__main__':
    ...
    # Set the total length of the training clips based on generated positive WAVs.
    # Use only WAV files and a fixed random seed so reruns remain reproducible.
    n = 50  # sample size
    positive_clips = get_wav_paths(positive_test_output_dir)
    rng = np.random.default_rng(12345)
    sample_count = min(n, len(positive_clips))
    sampled_clips = rng.choice(positive_clips, size=sample_count, replace=False)
    duration_in_samples = []
    for clip_path in sampled_clips:
        sr, dat = scipy.io.wavfile.read(clip_path)
        if sr != 16000:
            raise ValueError(f"Expected 16 kHz WAV, got {sr} Hz: {clip_path}")
    ...
    if args.augment_clips is True:
        if not os.path.exists(os.path.join(feature_save_dir, "positive_features_train.npy")) or args.overwrite is True:
            positive_clips_train = get_wav_paths(positive_train_output_dir) * config["augmentation_rounds"]
            ...
            positive_clips_test = get_wav_paths(positive_test_output_dir) * config["augmentation_rounds"]
            positive_clips_test_generator = augment_clips(positive_clips_test, total_length=config["total_length"],
                                                          batch_size=config["augmentation_batch_size"],
                                                          background_clip_paths=background_paths,
                                                          RIR_paths=rir_paths)

            negative_clips_train = get_wav_paths(negative_train_output_dir) * config["augmentation_rounds"]
            negative_clips_train_generator = augment_clips(negative_clips_train, total_length=config["total_length"],
                                                           batch_size=config["augmentation_batch_size"],
                                                           background_clip_paths=background_paths,
                                                           RIR_paths=rir_paths)

            negative_clips_test = get_wav_paths(negative_test_output_dir) * config["augmentation_rounds"]
    ...
    if args.train_model is True:
        ...
        if input_shape[1] != 96:
            raise ValueError(
                f"Unexpected openWakeWord feature dimension: {input_shape[1]}. "
                "This training script expects 96."
            )

        # Add generated positive and adversarial negative clips to the feature data files dictionary.
        # Only these configured feature files participate in training; unrelated/stale .npy
        # files in the directory are deliberately ignored.
        ...
        # Validate every source that will be given to mmap_batch_generator.
        # Auxiliary negative datasets (for example ACAV100M) may legitimately use
        # a different number of feature frames and are normalized by the transform
        # below. The generated positive/negative arrays must match the model exactly.
        for feature_key, feature_path in config["feature_data_files"].items():
            if feature_key in ("positive", "adversarial_negative"):
                validate_feature_file(
                    feature_path,
                    expected_timesteps=input_shape[0],
        ...
        # Every source gets the same fixed-shape transform. This prevents a stray
        # 34/35-frame source from reaching np.vstack() inside mmap_batch_generator.
        feature_transform = make_fixed_feature_transform(
            target_timesteps=input_shape[0],
            feature_dim=input_shape[1]
        )
        data_transforms = {
            key: feature_transform for key in config["feature_data_files"].keys()
        }

        # Create label transforms for the binary classifier.
        label_transforms = {
            key: (lambda x: [1 for _ in x]) if key == "positive"
        ...
        logging.info(
            "Training feature sources: " +
            ", ".join(f"{k}={v}" for k, v in config["feature_data_files"].items())
        )
        ...
        if X_val_fp.ndim != 2 or X_val_fp.shape[1] != input_shape[1]:
            raise ValueError(
                f"False-positive validation features have shape {X_val_fp.shape}; "
                f"expected (frames, {input_shape[1]})."
            )

        # Slide a fixed-size window with step 1 and include the final valid window.
        if X_val_fp.shape[0] < input_shape[0]:
            padded = np.zeros(
                (input_shape[0], input_shape[1]),
                dtype=X_val_fp.dtype
            )
            padded[:X_val_fp.shape[0]] = X_val_fp
            X_val_fp = padded[None, ...]
        else:
            X_val_fp = np.array([
                X_val_fp[i:i + input_shape[0]]
                for i in range(0, X_val_fp.shape[0] - input_shape[0] + 1, 1)
            ])
        ...
        if X_val_pos.ndim != 3 or X_val_neg.ndim != 3:
            raise ValueError("Validation feature arrays must be 3-dimensional.")

        if X_val_pos.shape[1:] != input_shape or X_val_neg.shape[1:] != input_shape:
            raise ValueError(
                "Validation feature shape mismatch: "
                f"positive={X_val_pos.shape}, negative={X_val_neg.shape}, "
                f"expected (*, {input_shape[0]}, {input_shape[1]})."
            )
