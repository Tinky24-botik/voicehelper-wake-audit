...
def compute_features_from_generator(generator, n_total, clip_duration, output_file, device="cpu", ncpu=1):
    ...
    # Закрываем handle 'fp' ДО вызова trim_mmap() — на Windows файл
    # остаётся заблокированным, пока этот handle открыт, поэтому
    # os.remove() внутри trim_mmap() падал с PermissionError.
